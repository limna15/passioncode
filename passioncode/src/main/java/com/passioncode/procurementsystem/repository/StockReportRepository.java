...
public interface StockReportRepository extends JpaRepository<MaterialOut, Integer> {
	 ...
	 * @return
	 */
	@Query(value="SELECT  t1.날짜 날짜,m.middle_category_code 중분류코드,mc.category 중분류이름, sum(t1.재고금액) 재고금액 "
			+ "FROM "
			+ "(select 날짜, 품목코드, 입고량, 입고금액, 출고량, 출고금액, 재고량,재고금액,if( 재고량 =0, 0, 재고단가) 재고단가 "
			+ "FROM "
			+ "(SELECT 날짜, 품목코드, 입고량, 입고금액, 출고량, 출고금액, if(입고량-출고량 <=0, 0, 입고량-출고량) 재고량,if(입고금액-출고금액 <=0,0, 입고금액-출고금액) 재고금액, round(nvl((입고금액-출고금액)/(입고량-출고량),0),-1)  재고단가 "
			...
			+ "JOIN material m ON m.code=t1.품목코드 "
			+ "JOIN middle_category mc ON m.middle_category_code=mc.code "
			+ "GROUP BY mc.code,t1.날짜", nativeQuery = true)
	public List<Object[]> getCalculStockReportForMC(@Param("startDate") String startDate,@Param("endDate") String endDate);	
